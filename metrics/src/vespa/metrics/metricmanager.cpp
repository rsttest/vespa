...
MetricManager::createSnapshotPeriods(const Config& config)
{
    ...
    try {
        for (auto length : config.snapshot.periods) {
            ...
            result.emplace_back(vespalib::from_s<system_time::duration>(length), name.str());
        }
    ...
    if (_snapshots.empty()) {
        ...
        for (uint32_t i = 0; i< snapshotPeriods.size(); ++i) {
            ...
            if (i + 1 < snapshotPeriods.size()) {
                ...
                if ((snapshotPeriods[i + 1].first % snapshotPeriods[i].first) != system_time::duration::zero()) {
