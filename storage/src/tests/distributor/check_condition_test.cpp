...
public:
    ...
    std::shared_ptr<api::GetCommand> sent_get_command(size_t idx) {
        ...
        auto cmd = std::dynamic_pointer_cast<api::GetCommand>(_sender.command(idx));
        assert(cmd != nullptr);
        return cmd;
    }
    ...
    void do_test_2_gets(const std::function<void(CheckCondition&)>& reply_invoker,
...
TEST_F(CheckConditionTest, no_checker_returned_when_probing_no_supported_on_at_least_one_node) {
...
TEST_F(CheckConditionTest, completed_checker_with_not_found_outcome_returned_if_empty_replica_set) {
