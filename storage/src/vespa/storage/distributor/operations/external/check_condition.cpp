...
bool
CheckCondition::all_nodes_support_document_condition_probe(std::span<const uint16_t> nodes,
                                                           const DistributorStripeOperationContext& op_ctx)
{
    ...
    for (uint16_t node : nodes) {
        if (!features_repo.node_supported_features(node).document_condition_probe) {
            return false;
        }
    }
    return true;
                                                ...
                                                PersistenceOperationMetricSet& metric)
{
    ...
    if (entries.empty()) {
        return create_not_found(bucket_space, node_ctx);
    }
    if (bucket_has_consistent_replicas(entries)) {
        return {}; // Replicas are consistent; no need for write-repair
    }
    if (!all_nodes_support_document_condition_probe(entries[0]->getNodes(), op_ctx)) {
