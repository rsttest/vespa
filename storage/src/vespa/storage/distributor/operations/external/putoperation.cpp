...
PutOperation::onStart(DistributorStripeMessageSender& sender)
{
    ...
    if (!has_condition()) {
        ...
        maybe_start_write_repair(sender);
    }
}

void PutOperation::maybe_start_write_repair(DistributorStripeMessageSender& sender) {
                                           ...
                                           DistributorStripeMessageSender& sender)
{
    if (outcome.matched_condition()) {
        ...
        _tracker.fail(sender, outcome.error_code());
    } else {
        _tracker.fail(sender, api::ReturnCode(api::ReturnCode::TEST_AND_SET_CONDITION_FAILED,
