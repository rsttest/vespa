...
class ServiceDumpReport extends BaseReport {
    ...
    @JsonInclude(JsonInclude.Include.NON_NULL)
    @JsonIgnoreProperties(ignoreUnknown = true)
    public record DumpOptions(@JsonGetter(CALL_GRAPH_RECORDING_FIELD) Boolean callGraphRecording,
                              @JsonGetter(DURATION_FIELD) Double duration,
                              @JsonGetter(SEND_PROFILING_SIGNAL_FIELD) Boolean sendProfilingSignal) {
